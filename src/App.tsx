...
// Machine-readable rates, separate from the prose block above. Prompts need the
// prose; the cost panel and the plausibility guard need real numbers.
let FX_RATES:Record<string,number>={};
 
// The Research Desk once wrote "$1,041/ton (approximately Rs 8,645 at 8.3 INR/USD)".
// The live rate was 95.75, shown on the user's own screen at that moment. Every
// downstream figure - revenue, margin, break-even - was then wrong by 11x.
// This block is injected into EVERY prompt that can produce a currency figure.
function fxRuleBlock():string{
  const usd=FX_RATES.USD;
  if(!usd)return "\n\nCURRENCY RULE: no live exchange rate is available in this session. Do NOT convert between currencies. Quote every figure in the currency its source used, and say the conversion was not possible.\n";
  const lo=(usd*0.9).toFixed(0), hi=(usd*1.1).toFixed(0);
  return "\n\nCURRENCY RULE (non-negotiable, live rate):\n"
    +"USD/INR = "+usd.toFixed(2)+" as of "+(LIVE_RATES.ts||new Date().toISOString().slice(0,10))+".\n"
    +"- Use THIS rate for every USD-to-INR conversion. Do not use a remembered or assumed rate.\n"
    +"- Show your working: $X * "+usd.toFixed(2)+" = Rs Y. A conversion with no working is not acceptable.\n"
    +"- SANITY CHECK: any USD-to-INR conversion must land between "+lo+"x and "+hi+"x the dollar figure. If your result falls outside that band you have made an arithmetic error - recompute before writing it down.\n"
    +"- Indian figures use lakh and crore (1 lakh = 100,000; 1 crore = 10,000,000). Never write 'Rs 8,645 crore' when you mean 'Rs 8,645'.\n";
}
 
// Deterministic post-check. Flags conversions outside the plausible band so a 10x
// error is visible to the reader instead of silently becoming the basis of a
// recommendation. Text-only: it annotates, it never rewrites the numbers.
function fxPlausibilityWarnings(text:string):string{
  try{
    const usd=FX_RATES.USD;
    if(!usd||!text)return "";
    const bad:string[]=[];
    const re=/\$\s?([\d,]+(?:\.\d+)?)\s*(million|billion|bn|mn)?[^.\n]{0,60}?(?:\u20b9|Rs\.?\s?)\s?([\d,]+(?:\.\d+)?)\s*(lakh|crore|cr|million|billion)?/gi;
    let m:any;
    while((m=re.exec(text))!==null&&bad.length<6){
      const mult=(s:string)=>({million:1e6,mn:1e6,billion:1e9,bn:1e9,lakh:1e5,crore:1e7,cr:1e7} as any)[String(s||"").toLowerCase()]||1;
      const d=parseFloat(m[1].replace(/,/g,""))*mult(m[2]);
      const r=parseFloat(m[3].replace(/,/g,""))*mult(m[4]);
      if(!isFinite(d)||!isFinite(r)||d<=0||r<=0)continue;
      const implied=r/d;
      if(implied<usd*0.6||implied>usd*1.6){
        bad.push("$"+m[1]+(m[2]?" "+m[2]:"")+" stated as Rs "+m[3]+(m[4]?" "+m[4]:"")+" implies a rate of "+implied.toFixed(1)+" (live rate is "+usd.toFixed(2)+")");
      }
    }
    if(!bad.length)return "";
    return "\n\n---\n\u26a0 **CURRENCY CHECK FAILED** - these conversions do not match the live rate and should not be relied on:\n"+bad.map(b=>"- "+b).join("\n")+"\n";
  }catch{return "";}
}
...
async function fetchLiveRates(){
  ...
  try{
    ...
    FX_RATES={
      USD: fx?.rates?.USD?1/fx.rates.USD:0,
      AED: fx?.rates?.AED?1/fx.rates.AED:0,
      GBP: fx?.rates?.GBP?1/fx.rates.GBP:0,
      EUR: fx?.rates?.EUR?1/fx.rates.EUR:0,
    };
    LIVE_RATES={
      loaded:true,ts,
...
// Real, working VBA. Not a placeholder and not a description of macros - this
// is the module a user imports into the workbook we generate.
function VBA_MODULE_SOURCE(name:string):string{
  return [
    "Attribute VB_Name = \"OrchestrIQ_Automation\"",
    "' " + name + " - generated by OrchestrIQ",
    "' Import: Excel > Alt+F11 > File > Import File > select this .bas",
    "Option Explicit",
    "",
    "Public Sub RefreshAll_Safe()",
    "    ' Refreshes every query and pivot, with the screen frozen so it is quick.",
    "    On Error GoTo Fail",
    ...
    "End Sub",
    "",
    "Public Sub RunControlChecks()",
    "    ' Compares every control total against its source and reports differences.",
    "    Dim ws As Worksheet, bad As Long",
    "    On Error Resume Next",
    "    Set ws = ThisWorkbook.Worksheets(\"Audit\")",
    "    On Error GoTo 0",
    "    If ws Is Nothing Then",
    "        MsgBox \"No Audit sheet found in this workbook.\", vbExclamation",
    "        Exit Sub",
    "    End If",
    "    Dim c As Range",
    "    For Each c In ws.UsedRange.Columns(3).Cells",
    "        If IsNumeric(c.Value) Then",
    "            If Abs(c.Value) > 0.005 Then bad = bad + 1",
    "        End If",
    "    Next c",
    "    Call StampRun(\"Checks\")",
    "    If bad = 0 Then",
    "        MsgBox \"All control checks passed.\", vbInformation",
    "    Else",
    "        MsgBox bad & \" check(s) did not reconcile. See the Audit sheet.\", vbExclamation",
    "    End If",
    "End Sub",
    "",
    "Public Sub ExportReportPDF()",
    "    ' Saves the Dashboard as a PDF next to this workbook.",
    "    On Error GoTo Fail",
    "    Dim p As String",
    "    p = ThisWorkbook.Path & Application.PathSeparator & _",
    "        Replace(ThisWorkbook.Name, \".xlsm\", \"\") & \"-\" & Format(Now, \"yyyymmdd-hhnn\") & \".pdf\"",
    "    ThisWorkbook.Worksheets(\"Dashboard\").ExportAsFixedFormat xlTypePDF, p",
    ...
    "End Sub",
    "",
    "Private Sub StampRun(what As String)",
    "    ' Leaves an audit trail of who ran what and when.",
    "    Dim ws As Worksheet",
    "    On Error Resume Next",
    "    Set ws = ThisWorkbook.Worksheets(\"Audit\")",
    "    On Error GoTo 0",
    ...
    "    ws.Cells(r, 2).Value = what",
    "    ws.Cells(r, 3).Value = Environ$(\"USERNAME\")",
    "End Sub",
  ].join("\r\n");
}
 
function VBA_README(name:string):string{
  return [
    "# " + name + " - macros",
    "",
    "## Read this first",
    "",
    "The workbook in this folder is a genuine .xlsx file. It is NOT macro-enabled,",
    "and we have deliberately not renamed it to .xlsm to pretend otherwise.",
    "",
    "A macro-enabled workbook has to contain a compiled VBA project, which can only",
    "be produced by Excel itself on Windows. Nothing in a web application can create",
    "one. Any tool that hands you a .xlsm without that inside is giving you a file",
    "that Excel will refuse or open with the macros missing.",
    "",
    "So the macros are supplied as source, to import in about thirty seconds.",
    "",
    "## How to enable them",
    "",
    "1. Open the .xlsx workbook in Excel on Windows.",
    "2. Save As, and choose **Excel Macro-Enabled Workbook (*.xlsm)**.",
    "3. Press **Alt + F11** to open the VBA editor.",
    "4. **File > Import File**, and choose the `-Macros.bas` file in this folder.",
    "5. Close the editor. Press **Alt + F8** to see the macros.",
    "6. Save again, keeping the .xlsm format.",
    "",
    "## What each macro does",
    "",
    "| Macro | What it does |",
    "|---|---|",
    "| RefreshAll_Safe | Refreshes every query and pivot, then recalculates. Screen updating is turned off so it is fast, and turned back on even if something fails. |",
    "| RunControlChecks | Reads the Audit sheet and reports any control total that does not reconcile to within 0.005. |",
    "| ExportReportPDF | Saves the Dashboard sheet as a timestamped PDF beside the workbook. |",
    "",
    "## Adding buttons",
    "",
    "Developer tab > Insert > Button (Form Control), draw it on the sheet, and pick",
    "the macro. If you cannot see the Developer tab: File > Options > Customise",
    "Ribbon > tick Developer.",
    "",
    "## Requirements",
    "",
    "- Microsoft Excel for Windows, 2021 or Microsoft 365",
    "- Macros enabled for this file (Excel will prompt on first open)",
    "- Sheets named Dashboard and Audit for the second and third macros to work",
    "",
    "## If something goes wrong",
    "",
    "- **Macros greyed out**: the file is still .xlsx. Redo step 2.",
    "- **\"Cannot run the macro\"**: the import did not complete. Repeat step 4.",
    "- **Export says failed**: there is no sheet named Dashboard in this workbook.",
  ].join("\n");
}
...
const S={
  lbl:{fontSize:11,fontWeight:800,color:"#8A8378",textTransform:"uppercase",letterSpacing:"0.09em",marginBottom:8,display:"block"},
  inp:{width:"100%",background:"#FFFFFF",border:"1px solid #DDD8CE",borderRadius:9,padding:"13px 15px",color:"#1C2A24",fontSize:13.5,fontFamily:"Inter,system-ui,sans-serif"},
  pBtn:{background:"#C9A961",color:"#1C2A24",border:"none",borderRadius:7,padding:"11px 20px",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",marginTop:8,width:"100%"},
  hBtn:{background:"#FFFFFF",border:"1px solid #DDD8CE",borderRadius:7,padding:"8px 14px",color:"#4A453D",fontSize:12.5,fontWeight:600,cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif"},
  iBtn:{background:"#FFFFFF",border:"1px solid #DDD8CE",borderRadius:7,padding:"8px 13px",color:"#4A453D",fontSize:12.5,fontWeight:600,cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif"},
  cancelBtn:{background:"#FAE6E2",border:"1px solid #E8C4BC",borderRadius:7,padding:"8px 14px",color:"#9E3B2B",fontSize:12.5,fontWeight:700,cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif"},
  actBtn:{background:"#FFFFFF",border:"1px solid #DDD8CE",borderRadius:7,padding:"11px 14px",color:"#4A453D",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",textAlign:"center",width:"100%"},
  errB:{background:"#FAE6E2",border:"1px solid #E8C4BC",borderRadius:8,padding:"11px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,fontSize:12.5,color:"#9E3B2B",gap:10},
  retBtn:{background:"#B54A3A",color:"#fff",border:"none",borderRadius:5,padding:"5px 11px",fontSize:11.5,fontWeight:700,cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",whiteSpace:"nowrap"},
  dot:{width:6,height:6,borderRadius:"50%",background:"#4C7A5E",animation:"pulse 1s ease-in-out infinite",display:"inline-block"},
  uMsg:{background:"#F2EFE9",borderRadius:10,padding:"15px 18px",border:"1px solid #E4E0D8",color:"#4A453D"},
  aMsg:{background:"#FFFFFF",borderRadius:10,padding:"15px 18px",border:"1px solid #E4E0D8",color:"#4A453D"},
  mLbl:{fontSize:12,fontWeight:700,color:"#1C2A24",marginBottom:8,textTransform:"none",letterSpacing:"0"},
  qaBtn:{background:"#FFFFFF",border:"1px solid #E4E0D8",borderRadius:9,padding:"13px 15px",color:"#4A453D",fontSize:13,cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",textAlign:"left",lineHeight:1.55},
  pill:{background:"#FFFFFF",border:"1px solid #DDD8CE",borderRadius:999,padding:"7px 13px",fontSize:12,fontWeight:600,color:"#6B6459",cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",flex:1,textAlign:"center"},
  dH:{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"none",border:"none",cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",borderRadius:7},
  rBtn:{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"9px 12px 9px 22px",background:"none",border:"1px solid transparent",cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",borderRadius:7,textAlign:"left"},
  nTab:{display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"#1C2A24",border:"1px solid transparent",borderRadius:6,padding:"7px 4px",color:"#7E9689",cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",fontSize:10,fontWeight:600},
  nrvTab:{display:"flex",flexDirection:"column",alignItems:"center",gap:2,flex:1,background:"#1C2A24",border:"1px solid transparent",borderRadius:6,padding:"7px 4px",color:"#7E9689",cursor:"pointer",fontFamily:"Inter,system-ui,sans-serif",fontSize:10,fontWeight:600},
  inpA:{padding:"14px 20px 16px",borderTop:"1px solid #E4E0D8",background:"#FBFAF8"},
  inpR:{display:"flex",alignItems:"flex-end",gap:6,background:"#FFFFFF",border:"1px solid #DDD8CE",borderRadius:9,padding:"4px 4px 4px 14px"},
  ta:{flex:1,background:"none",border:"none",color:"#1C2A24",fontSize:13.5,fontFamily:"Inter,system-ui,sans-serif",resize:"none",padding:"9px 0",lineHeight:1.6},
  sBtn:{width:32,height:32,borderRadius:7,border:"none",color:"#1C2A24",fontSize:15,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},
  modalBg:{position:"fixed",inset:0,background:"rgba(28,42,36,0.55)",backdropFilter:"blur(3px)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20},
  modal:{background:"#FBFAF8",border:"1px solid #DDD8CE",borderRadius:14,padding:"26px 28px",width:"100%",maxWidth:520,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 16px 46px rgba(60,50,35,.18)"},
};
...
const [wfPauseMsg,setWfPauseMsg]=useState("");
 ...
 useEffect(()=>{
  ...
  (async()=>{
    ...
    // fetchLiveRates() was DEFINED but never called anywhere in the app. LIVE_RATES
    // stayed {loaded:false}, so buildSys() always passed an empty rates block and no
    // executive ever saw a real exchange rate. The ticker fetched rates for DISPLAY
    // only; the prompts never received them. This one line is the root cause.
    try{fetchLiveRates();}catch{}
    setTimeout(()=>{
      initA11y();
  ...
  const runBR=useCallback(async()=>{
    ...
    try{
      ...
      if(!cancelRef.current.br&&res.length>0){
        ...
        try{const fxw=fxPlausibilityWarnings(syn);if(fxw)syn=fxw+"\n"+syn;}catch{}
...
Now produce the complete ${del.name}. Start with content immediately — no preamble.`;
  ...
  const runProjectPackage=useCallback(async(proj)=>{
    ...
    const _plan:ExecutionPlan={objectiveRestated:del.description||del.name,domain:(["finance","audit","strategy","marketing","operations","hr","legal","technology","sales","risk"] as const).find(d=>(del.capabilityType||"").toLowerCase().includes(d)||del.name.toLowerCase().includes(d))||"finance",persona:"Senior FP&A Director",audience:_delAudience(del),qualityStandard:"cfo_model",decisionContext:del.description||del.name,deliverables:[_spec],missingInfo:[],executionOrder:[del.name],validationCriteria:[]};
        ...
        // ── IMPROVED 2: Enterprise multi-sheet XLSX ──
          ...
          try{
            const _railwayUrl="https://orchestriq-gen-service-production.up.railway.app";
            const _allContent=(rawContentStore.current[del.id]||del.rawContent||"").replace(/^\s*(Here is|Here's|Below is)[^\n]*\n+/i,"");
            const _coCtx=[proj.context?.company?.name||co.name||"",proj.context?.company?.industry||co.industry||"",proj.context?.company?.stage||co.stage||"",proj.context?.company?.location||co.location||""].filter(Boolean).join(" | ");
            setProjectExecPhase("📊 Building CFO-grade Excel via Python engine: "+del.name+"...");
            const _r=await fetch(_railwayUrl+"/generate/excel",{
              method:"POST",
              headers:{"Content-Type":"application/json"},
              body:JSON.stringify({
                objective:del.description||del.name,
            ...
            // artefact - nothing in this pipeline can produce one.
            //
            // So we do the honest thing your own standard asks for: deliver a
            // genuine .xlsx, and put the macro source beside it as a .bas file
            // that imports into Excel in about thirty seconds.
            zip.folder(folder).file(fname+"-Macros.bas",VBA_MODULE_SOURCE(fname));
            zip.folder(folder).file(fname+"-HOW-TO-ENABLE-MACROS.md",VBA_README(fname));
          }catch(xlsxErr:any){
            // Fallback: SheetJS basic
            try{
    ...
    var _pdfContentSafe=pdfSafeText((content||"").replace(/^\s*---+\s*$/gm,""));const pdfBuf=await(async()=>{
          ...
          try{
            ...
            try{
              ...
              // runResearchDesk returns an object like {brief, grounded, provider}.
              // It was being glued straight into a string, so the file showed the
              // words "[object Object]" where the guidance should have been.
              const _rd:any=await runResearchDesk((s,m,t)=>ask(s,m,t,true),co,compData,researchQ,showToast,keys);
              alternativeGuide=typeof _rd==="string"?_rd:(_rd?.brief||"");
  ...
  // THIS ONE LINE IS WHY NO IMAGE WAS EVER PRODUCED.
  // This function reads keys.fal and keys.openai to generate images. But the
  // list below tells React which values the function is allowed to see. "keys"
  // was not in it - so the function kept using the EMPTY keys object from the
  // very first moment the page loaded, before your saved keys had arrived from
  // the database.
  //
  // Your fal.ai key was there. Your OpenAI key was there. The sidebar showed
      ...
      <div id="oiq-sidebar" className="oiq-sidebar" role="navigation" aria-label="Executive roster and module navigation" style={{width:210,background:"#0c1120",borderRight:"1px solid #14192a",display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden",paddingTop:'0'}}>
        ...
        <div style={{position:"relative",padding:"10px 10px",flexShrink:0}}>
          <button onClick={()=>setShowModules(v=>!v)}
            ...
            <span style={{flex:1,fontSize:11.5,fontWeight:700,color:"var(--oiq-sbText)",textAlign:"left",textTransform:"uppercase",letterSpacing:"0.06em"}}>{[["nerve","Nerve Center"],["jarvis","JARVIS"],["workflow","Workflow"],["agentic","Agentic AI"],["agents","AI Agents"],["p3","Autopilot"],["chat","Chat"],["data","Data Hub"],["costarch","Cost Architecture"],["home","Command Center"],["ledger","Ledger"],["finance","Finance"],["dispatch","Pulse"],["actions","Tasks"],["studio","Studio"],["funding","Funding"],["tokens","Tokens"],["workspace","AI Workspace"],["liveboard","Live Boardroom"],["account","My Account"],["admin","Admin Console"]].find(([v])=>v===view)?.[1]||"Nerve Center"}</span>
        ...
        {view==="chat"&&selRole&&curRole&&(
          <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 12px",borderBottom:"1px solid #14192a",background:"#0c1120"}}>
              ...
              <div style={{display:"flex",gap:4}}>
                {curMsgs.length>0&&<>
                  <ReadAloudButton text={curMsgs.filter(m=>m.role==="assistant").map(m=>stripMd(m.content)).join("\n\n")} id="chat-conversation" />
