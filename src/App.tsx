...
function buildSys(role,co,compData){
  ...
  return (
    ...
    "\n\nOUTPUT FORMAT — MANDATORY (McKinsey/BCG/Deloitte executive report standard):\nStructure every response using these exact sections in order:\n# Executive Summary\n(2-4 sentences: the single most important insight and recommended action)\n---\n## Key Insights\n(3-6 bullet points, each starting with a bold keyword)\n---\n## Detailed Analysis\n(logical sections with headers; use tables for any comparative data)\n---\n## Financial Impact\n(all figures in ; show formula → assumption → result for every number)\n---\n## Risks\n(bullet list; each risk has: likelihood, impact, mitigation)\n---\n## Opportunities\n(bullet list; each opportunity has: potential upside, timeframe, owner)\n---\n## Recommendations\n(table format: Priority | Action | Impact | Effort | Deadline)\n---\n## Sources & References\n(list every figure cited; format: [Source name] — [figure] — [date/status])\n---\nFORMATTING RULES (non-negotiable):\n- Use markdown headings, bullets, and tables throughout\n- Bold all key metrics, company names, and decision points\n- Never output walls of unbroken paragraph text\n- Never center content\n- Use whitespace generously between sections\n- Every number must have a unit ( or %)\n- Responses must be scannable in 90 seconds by a C-suite executive"
  );
}
  ...
  const runBR=useCallback(async()=>{
    ...
    try{
      ...
      if(!cancelRef.current.br&&res.length>0){
        ...
        const synSys="You are Chief of Staff at \""+co.name+"\"". "+buildCtx(co,compData)+researchContext+"\nSynthesize the boardroom debate into a board-ready consulting report. Use this EXACT format — every section required:\n\n# Executive Summary\n(3-4 sentences: single decision, headline number in "+synCur.sym+", recommended action)\n---\n## Key Insights\n(4-6 bullet points, each opening with a **bold keyword** — new synthesis only, never restate individual exec arguments)\n---\n## Points of Conflict\n| Leader | Position | Why It Matters |\n|--------|----------|----------------|\n(one row per disagreement found in the debate)\n---\n## Quantified Recommendation\n(single recommended path; show: formula → assumption → result for every figure in "+synCur.sym+")\n---\n## Financial Impact\n| Phase | Actions | Investment "+synCur.sym+" | Expected Return | Owner |\n|-------|---------|----------------|-----------------|-------|\n(30-60-90 day plan; one row per phase)\n---\n## Risk Register\n| Risk | Likelihood | Impact | Mitigation | Owner |\n|------|------------|--------|------------|-------|\n(max 5 rows)\n---\n## Opportunities\n(3-5 bullets: each with upside in "+synCur.sym+", timeframe, and owner)\n---\n## This Week's Decision\n(**Single action required now.** Cost of inaction: "+synCur.sym+" ___ per week. Decision owner: ___. Deadline: ___)\n---\n## Recommendations\n| Priority | Action | Impact | Effort | Deadline |\n|----------|--------|--------|--------|----------|\n(ranked 1–N)\n---\n## Sources & References\n(every figure cited: [Source] — [Figure] — [URL or ESTIMATE label])\n\nFORMATTING RULES:\n- Bold all key metrics, company names, decision points\n- Never write unbroken paragraph blocks\n- Every number must carry a unit ("+synCur.sym+" or %)\n- Under 2600 words total\n- All 10 sections must be present and complete\n- VERIFICATION: figures from VERIFIED RESEARCH BRIEF must cite source+URL; all others labeled ESTIMATE (unverified)";
